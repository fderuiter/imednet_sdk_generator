...
void
StudyList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			for (guint i=0;i<json_array_get_length(arr);i++) {
				...
				if (isprimitive("Study")) {
					jsonToValue(&inst, temp_json, "Study", "");
				} else {
...
StudyList::toJson()
{
	...
	if (isprimitive("Study")) {
		list<Study> new_list = static_cast<list <Study> > (getData());
		node = converttoJson(&new_list, "Study", "array");
	} else {
		...
		for (list<Study>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			Study obj = *it;
