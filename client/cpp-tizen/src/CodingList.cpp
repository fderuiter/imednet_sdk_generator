...
void
CodingList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<Coding> new_list;
			Coding inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("Coding")) {
					jsonToValue(&inst, temp_json, "Coding", "");
				} else {
...
CodingList::CodingList(char* json)
{
	this->fromJson(json);
}
...
CodingList::toJson()
{
	...
	if (isprimitive("Coding")) {
		list<Coding> new_list = static_cast<list <Coding> > (getData());
		node = converttoJson(&new_list, "Coding", "array");
	} else {
		...
		for (list<Coding>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			Coding obj = *it;
