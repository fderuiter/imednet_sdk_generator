...
static bool listVariablesProcessor(MemoryStruct_s p_chunk, long code, char* errormsg, void* userData,
	void(* voidHandler)())
{
...
static bool listVariablesHelper(char * accessToken,
	...
	, void* userData, bool isAsync)
{
	...
	string url("/studies/{studyKey}/variables");
	int pos;
	...
	if(!isAsync){
		...
		bool retval = listVariablesProcessor(*p_chunk, code, errormsg, userData,reinterpret_cast<void(*)()>(handler));
		...
		requestInfo = new(nothrow) RequestInfo (VariablesManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), listVariablesProcessor);;
		if(requestInfo == NULL)
			return false;
...
bool VariablesManager::listVariablesAsync(char * accessToken,
	std::string studyKey, int page, int size, std::string sort, std::string filter, 
	void(* handler)(VariableList, Error, void* )
	, void* userData)
{
	return listVariablesHelper(accessToken,
	studyKey, page, size, sort, filter, 
	handler, userData, true);
}

bool VariablesManager::listVariablesSync(char * accessToken,
	std::string studyKey, int page, int size, std::string sort, std::string filter, 
	void(* handler)(VariableList, Error, void* )
	, void* userData)
{
