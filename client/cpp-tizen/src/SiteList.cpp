...
void
SiteList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			for (guint i=0;i<json_array_get_length(arr);i++) {
				...
				if (isprimitive("Site")) {
					jsonToValue(&inst, temp_json, "Site", "");
				} else {
...
SiteList::toJson()
{
	...
	if (isprimitive("Site")) {
		list<Site> new_list = static_cast<list <Site> > (getData());
		node = converttoJson(&new_list, "Site", "array");
	} else {
		...
		for (list<Site>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			Site obj = *it;
