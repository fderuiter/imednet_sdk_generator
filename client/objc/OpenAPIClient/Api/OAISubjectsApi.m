...
#import "OAISubjectList.h"


@interface OAISubjectsApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation OAISubjectsApi

NSString* kOAISubjectsApiErrorDomain = @"OAISubjectsApiErrorDomain";
NSInteger kOAISubjectsApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) listSubjectsWithStudyKey: (NSString*) studyKey
    ...
    if (studyKey == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAISubjectsApiErrorDomain code:kOAISubjectsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/studies/{studyKey}/subjects"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAISubjectList*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAISubjectList*)data, error);
                                }
                            }];
}
