...
#import "OAISiteList.h"


@interface OAISitesApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation OAISitesApi

NSString* kOAISitesApiErrorDomain = @"OAISitesApiErrorDomain";
NSInteger kOAISitesApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) listSitesWithStudyKey: (NSString*) studyKey
    ...
    if (studyKey == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAISitesApiErrorDomain code:kOAISitesApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/studies/{studyKey}/sites"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAISiteList*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAISiteList*)data, error);
                                }
                            }];
}
