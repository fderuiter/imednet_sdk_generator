#import "OAIRecordRevisionsApi.h"
...
#import "OAIRecordRevisionList.h"


@interface OAIRecordRevisionsApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation OAIRecordRevisionsApi

NSString* kOAIRecordRevisionsApiErrorDomain = @"OAIRecordRevisionsApiErrorDomain";
NSInteger kOAIRecordRevisionsApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) listRecordRevisionsWithStudyKey: (NSString*) studyKey
    ...
    if (studyKey == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAIRecordRevisionsApiErrorDomain code:kOAIRecordRevisionsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/studies/{studyKey}/recordRevisions"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAIRecordRevisionList*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAIRecordRevisionList*)data, error);
                                }
                            }];
}
