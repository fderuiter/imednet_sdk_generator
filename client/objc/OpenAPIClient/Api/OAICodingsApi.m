#import "OAICodingsApi.h"
#import "OAIQueryParamCollection.h"
#import "OAIApiClient.h"
#import "OAICodingList.h"
...
@interface OAICodingsApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation OAICodingsApi

NSString* kOAICodingsApiErrorDomain = @"OAICodingsApiErrorDomain";
NSInteger kOAICodingsApiMissingParamErrorCode = 234513;
...
///
///  @returns OAICodingList*
///
-(NSURLSessionTask*) listCodingsWithStudyKey: (NSString*) studyKey
    page: (NSNumber*) page
    size: (NSNumber*) size
    sort: (NSString*) sort
    filter: (NSString*) filter
    completionHandler: (void (^)(OAICodingList* output, NSError* error)) handler {
    ...
    if (studyKey == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAICodingsApiErrorDomain code:kOAICodingsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/studies/{studyKey}/codings"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAICodingList*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAICodingList*)data, error);
                                }
                            }];
}
