...
#import "OAIVisitList.h"


@interface OAIVisitsApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation OAIVisitsApi

NSString* kOAIVisitsApiErrorDomain = @"OAIVisitsApiErrorDomain";
NSInteger kOAIVisitsApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) listVisitsWithStudyKey: (NSString*) studyKey
    ...
    if (studyKey == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAIVisitsApiErrorDomain code:kOAIVisitsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/studies/{studyKey}/visits"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAIVisitList*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAIVisitList*)data, error);
                                }
                            }];
}
