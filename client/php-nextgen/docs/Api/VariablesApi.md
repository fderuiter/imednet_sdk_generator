...
```php
listVariables($study_key, $page, $size, $sort, $filter): \OpenAPI\Client\Model\VariableList
```
...
try {
    $result = $apiInstance->listVariables($study_key, $page, $size, $sort, $filter);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling VariablesApi->listVariables: ', $e->getMessage(), PHP_EOL;
}
```
