...
```php
listVisits($study_key, $page, $size, $sort, $filter): \OpenAPI\Client\Model\VisitList
```
...
$apiInstance = new OpenAPI\Client\Api\VisitsApi(
...
try {
    $result = $apiInstance->listVisits($study_key, $page, $size, $sort, $filter);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling VisitsApi->listVisits: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\VisitList**](../Model/VisitList.md)
