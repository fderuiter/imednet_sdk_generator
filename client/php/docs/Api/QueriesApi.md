...
```php
listQueries($study_key, $page, $size, $sort, $filter): \OpenAPI\Client\Model\QueryList
```
...
try {
    $result = $apiInstance->listQueries($study_key, $page, $size, $sort, $filter);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling QueriesApi->listQueries: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\QueryList**](../Model/QueryList.md)
