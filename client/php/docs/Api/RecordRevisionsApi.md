# OpenAPI\Client\RecordRevisionsApi
...
```php
listRecordRevisions($study_key, $page, $size, $sort, $filter): \OpenAPI\Client\Model\RecordRevisionList
```
...
$apiInstance = new OpenAPI\Client\Api\RecordRevisionsApi(
...
try {
    $result = $apiInstance->listRecordRevisions($study_key, $page, $size, $sort, $filter);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling RecordRevisionsApi->listRecordRevisions: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\RecordRevisionList**](../Model/RecordRevisionList.md)
