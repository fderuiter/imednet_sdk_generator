# WWW::OpenAPIClient::SubjectsApi

## Load the API package
```perl
use WWW::OpenAPIClient::Object::SubjectsApi;
```
...
# **list_subjects**
> SubjectList list_subjects(study_key => $study_key, page => $page, size => $size, sort => $sort, filter => $filter)
...
eval {
    my $result = $api_instance->list_subjects(study_key => $study_key, page => $page, size => $size, sort => $sort, filter => $filter);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling SubjectsApi->list_subjects: $@\n";
}
```
