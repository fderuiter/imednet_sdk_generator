...
# Operation listSubjects → GET /studies/{studyKey}/subjects
...
func list_subjects(
	...
	if page < 0:
		...
		error.identifier = "list_subjects.param.validation.minimum"
	...
	if size > 500:
		var error := ApiError.new()
		error.identifier = "list_subjects.param.validation.maximum"
	...
	var bzz_path := "/api/v1/edc/studies/{studyKey}/subjects".replace("{" + "studyKey" + "}", _bzz_urlize_path_param(studyKey))
	...
	self._bzz_request(
		...
		func(bzz_response):
			bzz_response.data = SubjectList.bzz_denormalize_single(bzz_response.data)
...
func list_subjects_threaded(
	...
	var bzz_callable := Callable(self, "list_subjects")
	bzz_callable.bind(
		studyKey,
		page,
		size,
