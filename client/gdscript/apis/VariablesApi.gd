...
# Operation listVariables → GET /studies/{studyKey}/variables
...
func list_variables(
	...
	if page < 0:
		...
		error.identifier = "list_variables.param.validation.minimum"
	...
	if size > 500:
		var error := ApiError.new()
		error.identifier = "list_variables.param.validation.maximum"
	...
	var bzz_path := "/api/v1/edc/studies/{studyKey}/variables".replace("{" + "studyKey" + "}", _bzz_urlize_path_param(studyKey))
	...
	self._bzz_request(
		...
		func(bzz_response):
			bzz_response.data = VariableList.bzz_denormalize_single(bzz_response.data)
...
func list_variables_threaded(
	...
	var bzz_callable := Callable(self, "list_variables")
	bzz_callable.bind(
		studyKey,
		page,
		size,
