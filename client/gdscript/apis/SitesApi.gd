...
# Operation listSites → GET /studies/{studyKey}/sites
...
func list_sites(
	...
	if page < 0:
		...
		error.identifier = "list_sites.param.validation.minimum"
	...
	if size > 500:
		var error := ApiError.new()
		error.identifier = "list_sites.param.validation.maximum"
	...
	var bzz_path := "/api/v1/edc/studies/{studyKey}/sites".replace("{" + "studyKey" + "}", _bzz_urlize_path_param(studyKey))
	...
	self._bzz_request(
		...
		func(bzz_response):
			bzz_response.data = SiteList.bzz_denormalize_single(bzz_response.data)
...
func list_sites_threaded(
	...
	var bzz_callable := Callable(self, "list_sites")
	bzz_callable.bind(
		studyKey,
		page,
		size,
