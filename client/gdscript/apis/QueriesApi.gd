...
# Operation listQueries → GET /studies/{studyKey}/queries
...
func list_queries(
	...
	if page < 0:
		...
		error.identifier = "list_queries.param.validation.minimum"
	...
	if size > 500:
		var error := ApiError.new()
		error.identifier = "list_queries.param.validation.maximum"
	...
	var bzz_path := "/api/v1/edc/studies/{studyKey}/queries".replace("{" + "studyKey" + "}", _bzz_urlize_path_param(studyKey))
	...
	self._bzz_request(
		...
		func(bzz_response):
			bzz_response.data = QueryList.bzz_denormalize_single(bzz_response.data)
...
func list_queries_threaded(
	...
	var bzz_callable := Callable(self, "list_queries")
	bzz_callable.bind(
		studyKey,
		page,
		size,
