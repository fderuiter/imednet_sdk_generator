...
# Operation listStudies → GET /studies
...
func list_studies(
	...
	if page < 0:
		...
		error.identifier = "list_studies.param.validation.minimum"
	...
	if size > 500:
		var error := ApiError.new()
		error.identifier = "list_studies.param.validation.maximum"
	...
	var bzz_path := "/api/v1/edc/studies"
	...
	self._bzz_request(
		...
		func(bzz_response):
			bzz_response.data = StudyList.bzz_denormalize_single(bzz_response.data)
...
func list_studies_threaded(
	...
	var bzz_callable := Callable(self, "list_studies")
	bzz_callable.bind(
		page,
		size,
		sort,
