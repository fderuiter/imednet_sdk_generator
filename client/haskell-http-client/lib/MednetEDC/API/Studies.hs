...
module MednetEDC.API.Studies where
...
-- 
listStudies
  :: MednetEDCRequest ListStudies MimeNoContent StudyList MimeJSON
listStudies =
  _mkRequest "GET" ["/studies"]
...
instance HasOptionalParam ListStudies Sort2 where
...
instance HasOptionalParam ListStudies Filter where
  applyOptionalParam req (Filter xs) =
    req `addQuery` toQuery ("filter", Just xs)
-- | @application/json@
instance Produces ListStudies MimeJSON
