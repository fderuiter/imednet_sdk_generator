const samples = require('../samples/RecordRevisionsApi');
const RecordRevisionList = require('../models/RecordRevisionList');
...
module.exports = {
    listRecordRevisions: {
        key: 'listRecordRevisions',
        noun: 'Record Revisions',
        display: {
            label: 'List record revisions (audit trail entries) in a study',
            description: '',
            hidden: false,
        },
        operation: {
            ...
            outputFields: [
                ...RecordRevisionList.fields('', false),
            ],
            perform: async (z, bundle) => {
                const options = {
                    url: utils.replacePathParameters('https://edc.prod.imednetapi.com/api/v1/edc/studies/{studyKey}/recordRevisions'),
                    method: 'GET',
                ...
                return z.request(utils.requestOptionsMiddleware(z, bundle, options)).then((response) => {
                    ...
                    const results = utils.responseOptionsMiddleware(z, bundle, 'listRecordRevisions', response.json);
                    return results;
                })
            },
            sample: samples['RecordRevisionListSample']
        }
    },
}
