...
<a id="listForms"></a>
# **listForms**
> FormList listForms(studyKey, page, size, sort, filter)
...
try {
    val result : FormList = apiInstance.listForms(studyKey, page, size, sort, filter)
    println(result)
} catch (e: ClientException) {
    println("4xx response calling FormsApi#listForms")
    e.printStackTrace()
} catch (e: ServerException) {
    println("5xx response calling FormsApi#listForms")
    e.printStackTrace()
}
```
