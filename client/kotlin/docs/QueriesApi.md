...
try {
    val result : QueryList = apiInstance.listQueries(studyKey, page, size, sort, filter)
    println(result)
} catch (e: ClientException) {
    println("4xx response calling QueriesApi#listQueries")
    e.printStackTrace()
} catch (e: ServerException) {
    println("5xx response calling QueriesApi#listQueries")
    e.printStackTrace()
}
```
