...
try {
    val result : IntervalList = apiInstance.listIntervals(studyKey, page, size, sort, filter)
    println(result)
} catch (e: ClientException) {
    println("4xx response calling IntervalsApi#listIntervals")
    e.printStackTrace()
} catch (e: ServerException) {
    println("5xx response calling IntervalsApi#listIntervals")
    e.printStackTrace()
}
```
