...
import ../models/model_subject_list
...
proc listSubjects*(httpClient: HttpClient, studyKey: string, page: int, size: int, sort: string, filter: string): (Option[SubjectList], Response) =
  ...
  let response = httpClient.get(basepath & fmt"/studies/{studyKey}/subjects" & "?" & url_encoded_query_params)
  constructResult[SubjectList](response)
