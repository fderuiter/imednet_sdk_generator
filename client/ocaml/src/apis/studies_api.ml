...
let list_studies ?(page = 0l) ?(size = 25l) ?sort ?filter () =
    let open Lwt.Infix in
    let uri = Request.build_uri "/studies" in
    ...
    Request.read_json_body_as (JsonSupport.unwrap Study_list.of_yojson) resp body
