// tslint:disable
/**
 * Mednet EDC API
 * The Mednet EDC (Electronic Data Capture) REST API provides a single-point of access for reading data stored across iMednet data services. This specification documents all available endpoints in the latest version of the API. 
 *
 * The version of the OpenAPI document: 1.3.6
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import {
    Metadata,
    MetadataFromJSON,
    MetadataToJSON,
    Pagination,
    PaginationFromJSON,
    PaginationToJSON,
    Visit,
    VisitFromJSON,
    VisitToJSON,
} from './';

/**
 * 
 * @export
 * @interface VisitList
 */
export interface VisitList  {
    /**
     * 
     * @type {Metadata}
     * @memberof VisitList
     */
    metadata?: Metadata;
    /**
     * 
     * @type {Pagination}
     * @memberof VisitList
     */
    pagination?: Pagination;
    /**
     * 
     * @type {Array<Visit>}
     * @memberof VisitList
     */
    data?: Array<Visit>;
}

export function VisitListFromJSON(json: any): VisitList {
    return {
        'metadata': !exists(json, 'metadata') ? undefined : MetadataFromJSON(json['metadata']),
        'pagination': !exists(json, 'pagination') ? undefined : PaginationFromJSON(json['pagination']),
        'data': !exists(json, 'data') ? undefined : (json['data'] as Array<any>).map(VisitFromJSON),
    };
}

export function VisitListToJSON(value?: VisitList): any {
    if (value === undefined) {
        return undefined;
    }
    return {
        'metadata': MetadataToJSON(value.metadata),
        'pagination': PaginationToJSON(value.pagination),
        'data': value.data === undefined ? undefined : (value.data as Array<any>).map(VisitToJSON),
    };
}


