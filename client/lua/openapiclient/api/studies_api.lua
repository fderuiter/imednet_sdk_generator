...
local studies_api = {}
local studies_api_mt = {
	__name = "studies_api";
	__index = studies_api;
}

local function new_studies_api(authority, basePath, schemes)
	local schemes_map = {}
...
function studies_api:list_studies(page, size, sort, filter)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies?page=%s&size=%s&sort=%s&filter=%s",
			self.basePath, http_util.encodeURIComponent(page), http_util.encodeURIComponent(size), http_util.encodeURIComponent(sort), http_util.encodeURIComponent(filter));
	})
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_study_list.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
