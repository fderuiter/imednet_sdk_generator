...
local administration_api_mt = {
	__name = "administration_api";
	__index = administration_api;
}

local function new_administration_api(authority, basePath, schemes)
	local schemes_map = {}
	...
	return setmetatable({
		...
		api_key = {};
		access_token = nil;
	}, administration_api_mt)
end

function administration_api:list_users(study_key, page, size, sort, include_inactive)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies/%s/users?page=%s&size=%s&sort=%s&includeInactive=%s",
			self.basePath, study_key, http_util.encodeURIComponent(page), http_util.encodeURIComponent(size), http_util.encodeURIComponent(sort), http_util.encodeURIComponent(include_inactive));
	})
