...
-- model import
local openapiclient_site_list = require "openapiclient.model.site_list"
...
local function new_sites_api(authority, basePath, schemes)
	local schemes_map = {}
...
function sites_api:list_sites(study_key, page, size, sort, filter)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies/%s/sites?page=%s&size=%s&sort=%s&filter=%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_site_list.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
