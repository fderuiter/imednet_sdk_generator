...
-- model import
local openapiclient_interval_list = require "openapiclient.model.interval_list"
...
local intervals_api = {}
local intervals_api_mt = {
	__name = "intervals_api";
	__index = intervals_api;
}

local function new_intervals_api(authority, basePath, schemes)
	local schemes_map = {}
...
function intervals_api:list_intervals(study_key, page, size, sort, filter)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies/%s/intervals?page=%s&size=%s&sort=%s&filter=%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_interval_list.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
