...
final api_instance = SubjectsApi();
...
try {
    final result = api_instance.listSubjects(studyKey, page, size, sort, filter);
    print(result);
} catch (e) {
    print('Exception when calling SubjectsApi->listSubjects: $e\n');
}
```
