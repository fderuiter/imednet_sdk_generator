...
(defn-spec list-queries-with-http-info any?
  "List data queries in a study"
  ([studyKey string?, ] (list-queries-with-http-info studyKey nil))
  ([studyKey string?, {:keys [page size sort filter]} (s/map-of keyword? any?)]
   (check-required-params studyKey)
   (call-api "/studies/{studyKey}/queries" :get
...
(defn-spec list-queries query-list-spec
  "List data queries in a study"
  ([studyKey string?, ] (list-queries studyKey nil))
  ([studyKey string?, optional-params any?]
   (let [res (:data (list-queries-with-http-info studyKey optional-params))]
     (if (:decode-models *api-context*)
        (st/decode query-list-spec res st/string-transformer)
        res))))
