(ns mednet-edc-api.api.record-revisions
...
(defn-spec list-record-revisions-with-http-info any?
  "List record revisions (audit trail entries) in a study"
  ([studyKey string?, ] (list-record-revisions-with-http-info studyKey nil))
  ([studyKey string?, {:keys [page size sort filter]} (s/map-of keyword? any?)]
   (check-required-params studyKey)
   (call-api "/studies/{studyKey}/recordRevisions" :get
...
(defn-spec list-record-revisions record-revision-list-spec
  "List record revisions (audit trail entries) in a study"
  ([studyKey string?, ] (list-record-revisions studyKey nil))
  ([studyKey string?, optional-params any?]
   (let [res (:data (list-record-revisions-with-http-info studyKey optional-params))]
     (if (:decode-models *api-context*)
        (st/decode record-revision-list-spec res st/string-transformer)
        res))))
