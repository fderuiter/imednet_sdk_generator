...
(defn-spec get-job-status-with-http-info any?
  "Retrieve job status by batch ID"
  [studyKey string?, batchId string?]
  (check-required-params studyKey batchId)
  (call-api "/studies/{studyKey}/jobs/{batchId}" :get
            {:path-params   {"studyKey" studyKey "batchId" batchId }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec get-job-status job-spec
  "Retrieve job status by batch ID"
  [studyKey string?, batchId string?]
  (let [res (:data (get-job-status-with-http-info studyKey batchId))]
    (if (:decode-models *api-context*)
       (st/decode job-spec res st/string-transformer)
       res)))
