(ns mednet-edc-api.api.intervals
...
(defn-spec list-intervals-with-http-info any?
  "List intervals (visit definitions) in a study"
  ([studyKey string?, ] (list-intervals-with-http-info studyKey nil))
  ([studyKey string?, {:keys [page size sort filter]} (s/map-of keyword? any?)]
   (check-required-params studyKey)
   (call-api "/studies/{studyKey}/intervals" :get
...
(defn-spec list-intervals interval-list-spec
  "List intervals (visit definitions) in a study"
  ([studyKey string?, ] (list-intervals studyKey nil))
  ([studyKey string?, optional-params any?]
   (let [res (:data (list-intervals-with-http-info studyKey optional-params))]
     (if (:decode-models *api-context*)
        (st/decode interval-list-spec res st/string-transformer)
        res))))
