...
(defn-spec create-records-with-http-info any?
  "Add new record or update subject/record data"
  [studyKey string?, components-schemas-record-create-request-item (s/coll-of components-schemas-record-create-request-item-spec)]
  (check-required-params studyKey components-schemas-record-create-request-item)
  (call-api "/studies/{studyKey}/records" :post
            {:path-params   {"studyKey" studyKey }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    components-schemas-record-create-request-item
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["apiKeyAuth" "securityKeyAuth"]}))

(defn-spec create-records record-job-status-spec
  "Add new record or update subject/record data"
  [studyKey string?, components-schemas-record-create-request-item (s/coll-of components-schemas-record-create-request-item-spec)]
  (let [res (:data (create-records-with-http-info studyKey components-schemas-record-create-request-item))]
    (if (:decode-models *api-context*)
       (st/decode record-job-status-spec res st/string-transformer)
       res)))


(defn-spec list-records-with-http-info any?
  "List records (eCRF instances) in a study"
  ([studyKey string?, ] (list-records-with-http-info studyKey nil))
  ([studyKey string?, {:keys [page size sort filter recordDataFilter]} (s/map-of keyword? any?)]
   (check-required-params studyKey)
   (call-api "/studies/{studyKey}/records" :get
             {:path-params   {"studyKey" studyKey }
              :header-params {}
              :query-params  {"page" page "size" size "sort" sort "filter" filter "recordDataFilter" recordDataFilter }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["apiKeyAuth" "securityKeyAuth"]})))

(defn-spec list-records record-list-spec
  "List records (eCRF instances) in a study"
  ([studyKey string?, ] (list-records studyKey nil))
  ([studyKey string?, optional-params any?]
   (let [res (:data (list-records-with-http-info studyKey optional-params))]
     (if (:decode-models *api-context*)
        (st/decode record-list-spec res st/string-transformer)
        res))))
