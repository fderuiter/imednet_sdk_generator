...
export class CodingsApi {
    ...
    public setApiKey(key: CodingsApiApiKeys, value: string) {
        (this.authentications as any)[CodingsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async listCodings (studyKey: string, page?: number, size?: number, sort?: string, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CodingList;  }> {
        const localVarPath = this.basePath + '/studies/{studyKey}/codings'
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: CodingList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "CodingList");
