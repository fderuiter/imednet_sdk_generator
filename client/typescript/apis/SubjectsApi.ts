 ...
 * no description
 */
export class SubjectsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listSubjects(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'studyKey' is not null or undefined
        if (studyKey === null || studyKey === undefined) {
            throw new RequiredError("SubjectsApi", "listSubjects", "studyKey");
        }
...
export class SubjectsApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to listSubjects
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async listSubjectsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<SubjectList >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: SubjectList = ObjectSerializer.deserialize(
