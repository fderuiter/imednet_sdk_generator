...
export class AdministrationApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listUsers(studyKey: string, page?: number, size?: number, sort?: string, includeInactive?: boolean, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'studyKey' is not null or undefined
        if (studyKey === null || studyKey === undefined) {
            throw new RequiredError("AdministrationApi", "listUsers", "studyKey");
        }
        ...
        if (includeInactive !== undefined) {
            requestContext.setQueryParam("includeInactive", ObjectSerializer.serialize(includeInactive, "boolean", ""));
        }
...
export class AdministrationApiResponseProcessor {
     ...
     */
     public async listUsersWithHttpInfo(response: ResponseContext): Promise<HttpInfo<UserList >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: UserList = ObjectSerializer.deserialize(
