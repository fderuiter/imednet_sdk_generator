...
export class StudiesApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listStudies(page?: number, size?: number, sort?: string, filter?: string, _options?: Configuration): Promise<RequestContext> {
...
export class StudiesApiResponseProcessor {
     ...
     */
     public async listStudiesWithHttpInfo(response: ResponseContext): Promise<HttpInfo<StudyList >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: StudyList = ObjectSerializer.deserialize(
