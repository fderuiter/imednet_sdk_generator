...
export class FormsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listForms(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'studyKey' is not null or undefined
        if (studyKey === null || studyKey === undefined) {
            throw new RequiredError("FormsApi", "listForms", "studyKey");
        }
...
export class FormsApiResponseProcessor {
     ...
     */
     public async listFormsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<FormList >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: FormList = ObjectSerializer.deserialize(
