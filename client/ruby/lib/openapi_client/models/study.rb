  ...
  class Study
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Study` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Study`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'sponsor_key')
        self.sponsor_key = attributes[:'sponsor_key']
      end
      ...
      if attributes.key?(:'study_name')
        self.study_name = attributes[:'study_name']
      end

      if attributes.key?(:'study_description')
        self.study_description = attributes[:'study_description']
      end
