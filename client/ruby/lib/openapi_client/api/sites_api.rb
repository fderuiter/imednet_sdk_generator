...
module OpenapiClient
  class SitesApi
    ...
    # @return [SiteList]
    def list_sites(study_key, opts = {})
      data, _status_code, _headers = list_sites_with_http_info(study_key, opts)
      data
    end
    ...
    def list_sites_with_http_info(study_key, opts = {})
      ...
      if @api_client.config.client_side_validation && study_key.nil?
        fail ArgumentError, "Missing the required parameter 'study_key' when calling SitesApi.list_sites"
      end
      if @api_client.config.client_side_validation && !opts[:'page'].nil? && opts[:'page'] < 0
        fail ArgumentError, 'invalid value for "opts[:"page"]" when calling SitesApi.list_sites, must be greater than or equal to 0.'
      end

      if @api_client.config.client_side_validation && !opts[:'size'].nil? && opts[:'size'] > 500
        fail ArgumentError, 'invalid value for "opts[:"size"]" when calling SitesApi.list_sites, must be smaller than or equal to 500.'
      end

      if @api_client.config.client_side_validation && !opts[:'size'].nil? && opts[:'size'] < 1
        fail ArgumentError, 'invalid value for "opts[:"size"]" when calling SitesApi.list_sites, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/studies/{studyKey}/sites'.sub('{' + 'studyKey' + '}', CGI.escape(study_key.to_s))
