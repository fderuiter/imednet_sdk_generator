...
trait AdministrationApiEndpoints[F[*]] {

  def listUsers(studyKey: String, page: Option[Int] = None, size: Option[Int] = None, sort: Option[String] = None, includeInactive: Option[Boolean] = None)(using auth: _Authorization.ApiKey): F[UserList]

}

class AdministrationApiEndpointsImpl[F[*]: Concurrent](
  override val baseUrl: Uri,
  defaultHeaders: Seq[(String, String)] = Nil,
  httpClient: Http4sClient[F]
) extends BaseClient[F](baseUrl, defaultHeaders, httpClient) with AdministrationApiEndpoints[F] {
  
  import JsonSupports.*
  import io.circe.syntax.EncoderOps
  import cats.implicits.toFlatMapOps

  override def listUsers(studyKey: String, page: Option[Int] = None, size: Option[Int] = None, sort: Option[String] = None, includeInactive: Option[Boolean] = None)(using auth: _Authorization.ApiKey): F[UserList] = {
    ...
    val queryParameters = (
      ...
      includeInactive.map("includeInactive" -> _).map(Seq(_))
    ).toSeq.flatten

    _executeRequest[Unit, UserList](
      method = "GET",
      path = s"/studies/${studyKey}/users",
      body = None,
      formParameters = None,
      ...
      auth = Some(auth)) {
        ...
        case r if r.status.code == 200 => parseJson[F, UserList]("UserList", r)
