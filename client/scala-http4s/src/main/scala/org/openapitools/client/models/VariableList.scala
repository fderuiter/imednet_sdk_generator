...
case class VariableList(
    ...
    data: Option[Seq[Variable]] = None
)
  
object VariableList {
  given encoderVariableList: Encoder[VariableList] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderVariableList: Decoder[VariableList] = Decoder.instance { c =>
    for {
      metadata <- c.downField("metadata").as[Option[Metadata]]
      pagination <- c.downField("pagination").as[Option[Pagination]]
      data <- c.downField("data").as[Option[Seq[Variable]]]
    } yield VariableList(
      metadata = metadata,
