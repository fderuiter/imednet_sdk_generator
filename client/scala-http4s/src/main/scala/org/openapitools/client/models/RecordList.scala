...
case class RecordList(
    ...
    data: Option[Seq[Record]] = None
)
  
object RecordList {
  given encoderRecordList: Encoder[RecordList] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderRecordList: Decoder[RecordList] = Decoder.instance { c =>
    for {
      metadata <- c.downField("metadata").as[Option[Metadata]]
      pagination <- c.downField("pagination").as[Option[Pagination]]
      data <- c.downField("data").as[Option[Seq[Record]]]
    } yield RecordList(
      metadata = metadata,
