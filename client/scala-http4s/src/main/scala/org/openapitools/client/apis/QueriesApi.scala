...
trait QueriesApiEndpoints[F[*]] {

  def listQueries(studyKey: String, page: Option[Int] = None, size: Option[Int] = None, sort: Option[String] = None, filter: Option[String] = None)(using auth: _Authorization.ApiKey): F[QueryList]

}

class QueriesApiEndpointsImpl[F[*]: Concurrent](
  override val baseUrl: Uri,
  defaultHeaders: Seq[(String, String)] = Nil,
  httpClient: Http4sClient[F]
) extends BaseClient[F](baseUrl, defaultHeaders, httpClient) with QueriesApiEndpoints[F] {
  
  import JsonSupports.*
  import io.circe.syntax.EncoderOps
  import cats.implicits.toFlatMapOps

  override def listQueries(studyKey: String, page: Option[Int] = None, size: Option[Int] = None, sort: Option[String] = None, filter: Option[String] = None)(using auth: _Authorization.ApiKey): F[QueryList] = {
    ...
    _executeRequest[Unit, QueryList](
      method = "GET",
      path = s"/studies/${studyKey}/queries",
      body = None,
      formParameters = None,
      ...
      auth = Some(auth)) {
        ...
        case r if r.status.code == 200 => parseJson[F, QueryList]("QueryList", r)
