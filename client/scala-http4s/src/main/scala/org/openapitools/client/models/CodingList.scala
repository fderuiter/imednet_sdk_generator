...
case class CodingList(
    ...
    data: Option[Seq[Coding]] = None
)
  
object CodingList {
  given encoderCodingList: Encoder[CodingList] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderCodingList: Decoder[CodingList] = Decoder.instance { c =>
    for {
      metadata <- c.downField("metadata").as[Option[Metadata]]
      pagination <- c.downField("pagination").as[Option[Pagination]]
      data <- c.downField("data").as[Option[Seq[Coding]]]
    } yield CodingList(
      metadata = metadata,
