...
-- model import
local openapiclient_coding_list = require "openapiclient.model.coding_list"
...
local codings_api = {}
local codings_api_mt = {
	__name = "codings_api";
	__index = codings_api;
}

local function new_codings_api(authority, basePath, schemes)
	local schemes_map = {}
...
function codings_api:list_codings(study_key, page, size, sort, filter)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies/%s/codings?page=%s&size=%s&sort=%s&filter=%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_coding_list.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
