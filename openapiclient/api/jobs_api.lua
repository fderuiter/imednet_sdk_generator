...
local function new_jobs_api(authority, basePath, schemes)
	local schemes_map = {}
...
function jobs_api:get_job_status(study_key, batch_id)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies/%s/jobs/%s",
			self.basePath, study_key, batch_id);
	})
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_job.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
