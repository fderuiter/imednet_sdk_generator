...
-- model import
local openapiclient_subject_list = require "openapiclient.model.subject_list"
...
local subjects_api = {}
local subjects_api_mt = {
	__name = "subjects_api";
	__index = subjects_api;
}

local function new_subjects_api(authority, basePath, schemes)
	local schemes_map = {}
...
function subjects_api:list_subjects(study_key, page, size, sort, filter)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies/%s/subjects?page=%s&size=%s&sort=%s&filter=%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_subject_list.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
