...
-- model import
local openapiclient_form_list = require "openapiclient.model.form_list"
...
local function new_forms_api(authority, basePath, schemes)
	local schemes_map = {}
...
function forms_api:list_forms(study_key, page, size, sort, filter)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies/%s/forms?page=%s&size=%s&sort=%s&filter=%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_form_list.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
