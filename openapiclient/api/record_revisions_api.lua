...
-- model import
local openapiclient_record_revision_list = require "openapiclient.model.record_revision_list"
...
local record_revisions_api = {}
local record_revisions_api_mt = {
	__name = "record_revisions_api";
	__index = record_revisions_api;
}

local function new_record_revisions_api(authority, basePath, schemes)
	local schemes_map = {}
	...
	return setmetatable({
		...
		api_key = {};
		access_token = nil;
	}, record_revisions_api_mt)
end

function record_revisions_api:list_record_revisions(study_key, page, size, sort, filter)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies/%s/recordRevisions?page=%s&size=%s&sort=%s&filter=%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_record_revision_list.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
