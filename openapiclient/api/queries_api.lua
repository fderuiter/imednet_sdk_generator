...
-- model import
local openapiclient_query_list = require "openapiclient.model.query_list"
...
local queries_api = {}
local queries_api_mt = {
	__name = "queries_api";
	__index = queries_api;
}

local function new_queries_api(authority, basePath, schemes)
	local schemes_map = {}
...
function queries_api:list_queries(study_key, page, size, sort, filter)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/studies/%s/queries?page=%s&size=%s&sort=%s&filter=%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_query_list.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
