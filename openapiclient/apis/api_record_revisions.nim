...
import ../models/model_record_revision_list
...
proc listRecordRevisions*(httpClient: HttpClient, studyKey: string, page: int, size: int, sort: string, filter: string): (Option[RecordRevisionList], Response) =
  ...
  let response = httpClient.get(basepath & fmt"/studies/{studyKey}/recordRevisions" & "?" & url_encoded_query_params)
  constructResult[RecordRevisionList](response)
