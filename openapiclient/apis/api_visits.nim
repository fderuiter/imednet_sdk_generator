...
import ../models/model_visit_list
...
proc listVisits*(httpClient: HttpClient, studyKey: string, page: int, size: int, sort: string, filter: string): (Option[VisitList], Response) =
  ...
  let response = httpClient.get(basepath & fmt"/studies/{studyKey}/visits" & "?" & url_encoded_query_params)
  constructResult[VisitList](response)
