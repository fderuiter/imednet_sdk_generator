...
proc listSites*(httpClient: HttpClient, studyKey: string, page: int, size: int, sort: string, filter: string): (Option[SiteList], Response) =
  ...
  let response = httpClient.get(basepath & fmt"/studies/{studyKey}/sites" & "?" & url_encoded_query_params)
  constructResult[SiteList](response)
