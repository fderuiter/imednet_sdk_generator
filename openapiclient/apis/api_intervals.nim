...
import ../models/model_interval_list
...
proc listIntervals*(httpClient: HttpClient, studyKey: string, page: int, size: int, sort: string, filter: string): (Option[IntervalList], Response) =
  ...
  let response = httpClient.get(basepath & fmt"/studies/{studyKey}/intervals" & "?" & url_encoded_query_params)
  constructResult[IntervalList](response)
