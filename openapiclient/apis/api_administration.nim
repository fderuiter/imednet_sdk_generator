...
proc listUsers*(httpClient: HttpClient, studyKey: string, page: int, size: int, sort: string, includeInactive: bool): (Option[UserList], Response) =
  ...
  let url_encoded_query_params = encodeQuery([
    ...
    ("includeInactive", $includeInactive), # For user listing, whether to include inactive users
  ])

  let response = httpClient.get(basepath & fmt"/studies/{studyKey}/users" & "?" & url_encoded_query_params)
  constructResult[UserList](response)
