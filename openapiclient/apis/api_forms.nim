...
proc listForms*(httpClient: HttpClient, studyKey: string, page: int, size: int, sort: string, filter: string): (Option[FormList], Response) =
  ...
  let response = httpClient.get(basepath & fmt"/studies/{studyKey}/forms" & "?" & url_encoded_query_params)
  constructResult[FormList](response)
