...
void
IntervalList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			for (guint i=0;i<json_array_get_length(arr);i++) {
				...
				if (isprimitive("Interval")) {
					jsonToValue(&inst, temp_json, "Interval", "");
				} else {
...
IntervalList::IntervalList(char* json)
{
	this->fromJson(json);
}
...
IntervalList::toJson()
{
	...
	if (isprimitive("Interval")) {
		list<Interval> new_list = static_cast<list <Interval> > (getData());
		node = converttoJson(&new_list, "Interval", "array");
	} else {
		...
		for (list<Interval>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			Interval obj = *it;
