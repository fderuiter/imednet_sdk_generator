...
case class SubjectList(
    ...
    data: Option[Seq[Subject]] = None
)
  
object SubjectList {
  given encoderSubjectList: Encoder[SubjectList] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderSubjectList: Decoder[SubjectList] = Decoder.instance { c =>
    for {
      metadata <- c.downField("metadata").as[Option[Metadata]]
      pagination <- c.downField("pagination").as[Option[Pagination]]
      data <- c.downField("data").as[Option[Seq[Subject]]]
    } yield SubjectList(
      metadata = metadata,
