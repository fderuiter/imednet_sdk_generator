...
case class IntervalList(
    ...
    data: Option[Seq[Interval]] = None
)
  
object IntervalList {
  given encoderIntervalList: Encoder[IntervalList] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderIntervalList: Decoder[IntervalList] = Decoder.instance { c =>
    for {
      metadata <- c.downField("metadata").as[Option[Metadata]]
      pagination <- c.downField("pagination").as[Option[Pagination]]
      data <- c.downField("data").as[Option[Seq[Interval]]]
    } yield IntervalList(
      metadata = metadata,
