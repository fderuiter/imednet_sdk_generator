...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        ...
        if (requestConfig.headers["x-imn-security-key"].isNullOrEmpty()) {
            if (apiKey["x-imn-security-key"] != null) {
                if (apiKeyPrefix["x-imn-security-key"] != null) {
                    requestConfig.headers["x-imn-security-key"] = apiKeyPrefix["x-imn-security-key"]!! + " " + apiKey["x-imn-security-key"]!!
                } else {
                    requestConfig.headers["x-imn-security-key"] = apiKey["x-imn-security-key"]!!
                }
            }
        }
    }
