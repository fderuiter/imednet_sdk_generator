...
void
VisitList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			for (guint i=0;i<json_array_get_length(arr);i++) {
				...
				if (isprimitive("Visit")) {
					jsonToValue(&inst, temp_json, "Visit", "");
				} else {
...
VisitList::toJson()
{
	...
	if (isprimitive("Visit")) {
		list<Visit> new_list = static_cast<list <Visit> > (getData());
		node = converttoJson(&new_list, "Visit", "array");
	} else {
		...
		for (list<Visit>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			Visit obj = *it;
