#ifndef _StudiesManager_H_
#define _StudiesManager_H_
...
public:
	StudiesManager();
	virtual ~StudiesManager();

/*! \brief List studies accessible by API key. *Synchronous*
 *
 * 
 ...
 */
bool listStudiesSync(char * accessToken,
	int page, int size, std::string sort, std::string filter, 
	void(* handler)(StudyList, Error, void* )
	, void* userData);

/*! \brief List studies accessible by API key. *Asynchronous*
 *
 * 
 * \param page Page index to retrieve (0-based)
 * \param size Number of items to return per page (max 500)
 ...
 * \param userData The user data to be passed to the callback function.
 */
bool listStudiesAsync(char * accessToken,
	int page, int size, std::string sort, std::string filter, 
	void(* handler)(StudyList, Error, void* )
	, void* userData);
