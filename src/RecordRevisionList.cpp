...
void
RecordRevisionList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<RecordRevision> new_list;
			RecordRevision inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("RecordRevision")) {
					jsonToValue(&inst, temp_json, "RecordRevision", "");
				} else {
...
RecordRevisionList::RecordRevisionList(char* json)
{
	this->fromJson(json);
}
...
RecordRevisionList::toJson()
{
	...
	if (isprimitive("RecordRevision")) {
		list<RecordRevision> new_list = static_cast<list <RecordRevision> > (getData());
		node = converttoJson(&new_list, "RecordRevision", "array");
	} else {
		...
		for (list<RecordRevision>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			RecordRevision obj = *it;
