...
function Invoke-ListUsers {
    ...
    Process {
        ...
        $LocalVarUri = '/studies/{studyKey}/users'
        if (!$StudyKey) {
            throw "Error! The required parameter `StudyKey` missing when calling listUsers."
        }
        ...
        if ($IncludeInactive) {
            $LocalVarQueryParameters['includeInactive'] = $IncludeInactive
        }
