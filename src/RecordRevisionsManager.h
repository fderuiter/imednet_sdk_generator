#ifndef _RecordRevisionsManager_H_
#define _RecordRevisionsManager_H_
...
/** \addtogroup RecordRevisions RecordRevisions
 * \ingroup Operations
 *  @{
 */
class RecordRevisionsManager {
public:
	RecordRevisionsManager();
	virtual ~RecordRevisionsManager();

/*! \brief List record revisions (audit trail entries) in a study. *Synchronous*
 *
 * 
 ...
 */
bool listRecordRevisionsSync(char * accessToken,
	std::string studyKey, int page, int size, std::string sort, std::string filter, 
	void(* handler)(RecordRevisionList, Error, void* )
	, void* userData);

/*! \brief List record revisions (audit trail entries) in a study. *Asynchronous*
 *
 * 
 * \param studyKey Study key identifying the study context for the request *Required*
 * \param page Page index to retrieve (0-based)
 ...
 * \param userData The user data to be passed to the callback function.
 */
bool listRecordRevisionsAsync(char * accessToken,
	std::string studyKey, int page, int size, std::string sort, std::string filter, 
	void(* handler)(RecordRevisionList, Error, void* )
	, void* userData);
...
#endif /* RecordRevisionsManager_H_ */
