...
public interface RecordsApi  {
    ...
    @ApiOperation(value = "Add new record or update subject/record data", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 202, message = "Accepted request for record creation. Returns a job identifier to check status.", response = RecordJobStatus.class),
        ...
        @ApiResponse(code = 429, message = "Too many requests (rate limit exceeded)", response = InlineObject4.class),
        @ApiResponse(code = 500, message = "Internal server error (unexpected condition encountered)", response = InlineObject5.class) })
    public RecordJobStatus createRecords(@PathParam("studyKey") String studyKey, List<ComponentsSchemasRecordCreateRequestItem> componentsSchemasRecordCreateRequestItem);
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "List records (eCRF instances) in a study", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Successful response with list of records", response = RecordList.class),
        @ApiResponse(code = 400, message = "Bad request (malformed or invalid input)", response = InlineObject.class),
        @ApiResponse(code = 401, message = "Authentication failed or was not provided", response = InlineObject1.class),
        @ApiResponse(code = 403, message = "Authenticated but not allowed to access the resource", response = InlineObject2.class),
        @ApiResponse(code = 404, message = "The requested resource was not found", response = InlineObject3.class),
        @ApiResponse(code = 500, message = "Internal server error (unexpected condition encountered)", response = InlineObject5.class) })
    public RecordList listRecords(@PathParam("studyKey") String studyKey, @QueryParam("page") @DefaultValue("0")Integer page, @QueryParam("size") @DefaultValue("25")Integer size, @QueryParam("sort") String sort, @QueryParam("filter") String filter, @QueryParam("recordDataFilter") String recordDataFilter);
}
