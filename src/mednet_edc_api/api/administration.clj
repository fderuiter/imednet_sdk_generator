(ns mednet-edc-api.api.administration
...
(defn-spec list-users-with-http-info any?
  "List users and their roles in a study"
  ([studyKey string?, ] (list-users-with-http-info studyKey nil))
  ([studyKey string?, {:keys [page size sort includeInactive]} (s/map-of keyword? any?)]
   (check-required-params studyKey)
   (call-api "/studies/{studyKey}/users" :get
             {:path-params   {"studyKey" studyKey }
              :header-params {}
              :query-params  {"page" page "size" size "sort" sort "includeInactive" includeInactive }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["apiKeyAuth" "securityKeyAuth"]})))

(defn-spec list-users user-list-spec
  "List users and their roles in a study"
  ([studyKey string?, ] (list-users studyKey nil))
  ([studyKey string?, optional-params any?]
   (let [res (:data (list-users-with-http-info studyKey optional-params))]
     (if (:decode-models *api-context*)
        (st/decode user-list-spec res st/string-transformer)
        res))))
