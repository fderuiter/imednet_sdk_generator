(ns mednet-edc-api.api.variables
...
(defn-spec list-variables-with-http-info any?
  "List variables (fields) in a study"
  ([studyKey string?, ] (list-variables-with-http-info studyKey nil))
  ([studyKey string?, {:keys [page size sort filter]} (s/map-of keyword? any?)]
   (check-required-params studyKey)
   (call-api "/studies/{studyKey}/variables" :get
...
(defn-spec list-variables variable-list-spec
  "List variables (fields) in a study"
  ([studyKey string?, ] (list-variables studyKey nil))
  ([studyKey string?, optional-params any?]
   (let [res (:data (list-variables-with-http-info studyKey optional-params))]
     (if (:decode-models *api-context*)
        (st/decode variable-list-spec res st/string-transformer)
        res))))
