...
(defn-spec list-studies-with-http-info any?
  "List studies accessible by API key"
  ([] (list-studies-with-http-info nil))
  ([{:keys [page size sort filter]} (s/map-of keyword? any?)]
...
(defn-spec list-studies study-list-spec
  "List studies accessible by API key"
  ([] (list-studies nil))
  ([optional-params any?]
   (let [res (:data (list-studies-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode study-list-spec res st/string-transformer)
        res))))
