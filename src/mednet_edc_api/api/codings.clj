...
(defn-spec list-codings-with-http-info any?
  "List coding activities in a study"
  ([studyKey string?, ] (list-codings-with-http-info studyKey nil))
  ([studyKey string?, {:keys [page size sort filter]} (s/map-of keyword? any?)]
   (check-required-params studyKey)
   (call-api "/studies/{studyKey}/codings" :get
...
(defn-spec list-codings coding-list-spec
  "List coding activities in a study"
  ([studyKey string?, ] (list-codings studyKey nil))
  ([studyKey string?, optional-params any?]
   (let [res (:data (list-codings-with-http-info studyKey optional-params))]
     (if (:decode-models *api-context*)
        (st/decode coding-list-spec res st/string-transformer)
        res))))
