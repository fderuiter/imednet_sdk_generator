...
constructor({ accessToken, apiKeys, basePath, credentials, encodeParam, encoder, password, username, withCredentials }: ConfigurationParameters = {}) {
        ...
        // init default securityKeyAuth credential
        if (!this.credentials['securityKeyAuth']) {
            this.credentials['securityKeyAuth'] = () => {
                if (this.apiKeys === null || this.apiKeys === undefined) {
                    return undefined;
                } else {
                    return this.apiKeys['securityKeyAuth'] || this.apiKeys['x-imn-security-key'];
                }
            };
        }
    }
