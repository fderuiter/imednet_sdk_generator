...
module MednetEDC.API.Sites where
...
listSites
  ...
  -> MednetEDCRequest ListSites MimeNoContent SiteList MimeJSON
listSites (StudyKey studyKey) =
  _mkRequest "GET" ["/studies/",toPath studyKey,"/sites"]
...
instance HasOptionalParam ListSites Sort2 where
...
instance HasOptionalParam ListSites Filter where
  applyOptionalParam req (Filter xs) =
    req `addQuery` toQuery ("filter", Just xs)
-- | @application/json@
instance Produces ListSites MimeJSON
