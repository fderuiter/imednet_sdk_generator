...
module MednetEDC.API.Queries where
...
listQueries
  ...
  -> MednetEDCRequest ListQueries MimeNoContent QueryList MimeJSON
listQueries (StudyKey studyKey) =
  _mkRequest "GET" ["/studies/",toPath studyKey,"/queries"]
...
instance HasOptionalParam ListQueries Sort2 where
...
instance HasOptionalParam ListQueries Filter where
  applyOptionalParam req (Filter xs) =
    req `addQuery` toQuery ("filter", Just xs)
-- | @application/json@
instance Produces ListQueries MimeJSON
