...
module MednetEDC.API.Subjects where
...
listSubjects
  ...
  -> MednetEDCRequest ListSubjects MimeNoContent SubjectList MimeJSON
listSubjects (StudyKey studyKey) =
  _mkRequest "GET" ["/studies/",toPath studyKey,"/subjects"]
...
instance HasOptionalParam ListSubjects Sort2 where
...
instance HasOptionalParam ListSubjects Filter where
  applyOptionalParam req (Filter xs) =
    req `addQuery` toQuery ("filter", Just xs)
-- | @application/json@
instance Produces ListSubjects MimeJSON
