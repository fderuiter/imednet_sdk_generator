...
module MednetEDC.API.RecordRevisions where
...
listRecordRevisions
  ...
  -> MednetEDCRequest ListRecordRevisions MimeNoContent RecordRevisionList MimeJSON
listRecordRevisions (StudyKey studyKey) =
  _mkRequest "GET" ["/studies/",toPath studyKey,"/recordRevisions"]
    `_hasAuthType` (P.Proxy :: P.Proxy AuthApiKeyApiKeyAuth)
    `_hasAuthType` (P.Proxy :: P.Proxy AuthApiKeySecurityKeyAuth)

data ListRecordRevisions  

-- | /Optional Param/ "page" - Page index to retrieve (0-based)
instance HasOptionalParam ListRecordRevisions Page where
  applyOptionalParam req (Page xs) =
    req `addQuery` toQuery ("page", Just xs)

-- | /Optional Param/ "size" - Number of items to return per page (max 500)
instance HasOptionalParam ListRecordRevisions Size where
...
instance HasOptionalParam ListRecordRevisions Sort2 where
...
instance HasOptionalParam ListRecordRevisions Filter where
  applyOptionalParam req (Filter xs) =
    req `addQuery` toQuery ("filter", Just xs)
-- | @application/json@
instance Produces ListRecordRevisions MimeJSON
