...
module MednetEDC.API.Forms where
...
listForms
  ...
  -> MednetEDCRequest ListForms MimeNoContent FormList MimeJSON
listForms (StudyKey studyKey) =
  _mkRequest "GET" ["/studies/",toPath studyKey,"/forms"]
...
instance HasOptionalParam ListForms Sort2 where
...
instance HasOptionalParam ListForms Filter where
  applyOptionalParam req (Filter xs) =
    req `addQuery` toQuery ("filter", Just xs)
-- | @application/json@
instance Produces ListForms MimeJSON
