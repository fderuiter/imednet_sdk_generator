...
module MednetEDC.API.Visits where
...
listVisits
  ...
  -> MednetEDCRequest ListVisits MimeNoContent VisitList MimeJSON
listVisits (StudyKey studyKey) =
  _mkRequest "GET" ["/studies/",toPath studyKey,"/visits"]
...
instance HasOptionalParam ListVisits Sort2 where
...
instance HasOptionalParam ListVisits Filter where
  applyOptionalParam req (Filter xs) =
    req `addQuery` toQuery ("filter", Just xs)
-- | @application/json@
instance Produces ListVisits MimeJSON
