...
# Operation listIntervals → GET /studies/{studyKey}/intervals
...
func list_intervals(
	...
	if page < 0:
		...
		error.identifier = "list_intervals.param.validation.minimum"
	...
	if size > 500:
		var error := ApiError.new()
		error.identifier = "list_intervals.param.validation.maximum"
	...
	var bzz_path := "/api/v1/edc/studies/{studyKey}/intervals".replace("{" + "studyKey" + "}", _bzz_urlize_path_param(studyKey))
	...
	self._bzz_request(
		...
		func(bzz_response):
			bzz_response.data = IntervalList.bzz_denormalize_single(bzz_response.data)
...
func list_intervals_threaded(
	...
	var bzz_callable := Callable(self, "list_intervals")
	bzz_callable.bind(
		studyKey,
		page,
		size,
