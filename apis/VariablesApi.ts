...
export class VariablesApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listVariables(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'studyKey' is not null or undefined
        if (studyKey === null || studyKey === undefined) {
            throw new RequiredError("VariablesApi", "listVariables", "studyKey");
        }
...
export class VariablesApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to listVariables
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async listVariablesWithHttpInfo(response: ResponseContext): Promise<HttpInfo<VariableList >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: VariableList = ObjectSerializer.deserialize(
