...
# Operation listForms → GET /studies/{studyKey}/forms
...
func list_forms(
	...
	if page < 0:
		...
		error.identifier = "list_forms.param.validation.minimum"
	...
	if size > 500:
		var error := ApiError.new()
		error.identifier = "list_forms.param.validation.maximum"
	...
	var bzz_path := "/api/v1/edc/studies/{studyKey}/forms".replace("{" + "studyKey" + "}", _bzz_urlize_path_param(studyKey))
	...
	self._bzz_request(
		...
		func(bzz_response):
			bzz_response.data = FormList.bzz_denormalize_single(bzz_response.data)
...
func list_forms_threaded(
	...
	var bzz_callable := Callable(self, "list_forms")
	bzz_callable.bind(
		studyKey,
		page,
		size,
