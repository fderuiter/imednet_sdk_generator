...
# Operation listCodings → GET /studies/{studyKey}/codings
...
func list_codings(
	...
	if page < 0:
		...
		error.identifier = "list_codings.param.validation.minimum"
	...
	if size > 500:
		var error := ApiError.new()
		error.identifier = "list_codings.param.validation.maximum"
	...
	var bzz_path := "/api/v1/edc/studies/{studyKey}/codings".replace("{" + "studyKey" + "}", _bzz_urlize_path_param(studyKey))
	...
	self._bzz_request(
		...
		func(bzz_response):
			bzz_response.data = CodingList.bzz_denormalize_single(bzz_response.data)
...
func list_codings_threaded(
	...
	var bzz_callable := Callable(self, "list_codings")
	bzz_callable.bind(
		studyKey,
		page,
		size,
