 ...
 * no description
 */
export class RecordRevisionsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listRecordRevisions(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'studyKey' is not null or undefined
        if (studyKey === null || studyKey === undefined) {
            throw new RequiredError("RecordRevisionsApi", "listRecordRevisions", "studyKey");
        }
...
export class RecordRevisionsApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to listRecordRevisions
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async listRecordRevisionsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<RecordRevisionList >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: RecordRevisionList = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "RecordRevisionList", ""
            ) as RecordRevisionList;
