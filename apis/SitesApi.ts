...
export class SitesApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listSites(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'studyKey' is not null or undefined
        if (studyKey === null || studyKey === undefined) {
            throw new RequiredError("SitesApi", "listSites", "studyKey");
        }
...
export class SitesApiResponseProcessor {
     ...
     */
     public async listSitesWithHttpInfo(response: ResponseContext): Promise<HttpInfo<SiteList >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: SiteList = ObjectSerializer.deserialize(
