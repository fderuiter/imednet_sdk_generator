 ...
 * no description
 */
export class RecordsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async createRecords(studyKey: string, componentsSchemasRecordCreateRequestItem: Array<ComponentsSchemasRecordCreateRequestItem>, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'studyKey' is not null or undefined
        if (studyKey === null || studyKey === undefined) {
            throw new RequiredError("RecordsApi", "createRecords", "studyKey");
        }
        ...
        if (componentsSchemasRecordCreateRequestItem === null || componentsSchemasRecordCreateRequestItem === undefined) {
            throw new RequiredError("RecordsApi", "createRecords", "componentsSchemasRecordCreateRequestItem");
        }
        ...
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(componentsSchemasRecordCreateRequestItem, "Array<ComponentsSchemasRecordCreateRequestItem>", ""),
            contentType
        );
     ...
     */
    public async listRecords(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, recordDataFilter?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'studyKey' is not null or undefined
        if (studyKey === null || studyKey === undefined) {
            throw new RequiredError("RecordsApi", "listRecords", "studyKey");
        }
        ...
        if (recordDataFilter !== undefined) {
            requestContext.setQueryParam("recordDataFilter", ObjectSerializer.serialize(recordDataFilter, "string", ""));
        }
...
export class RecordsApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to createRecords
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async createRecordsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<RecordJobStatus >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("202", response.httpStatusCode)) {
            const body: RecordJobStatus = ObjectSerializer.deserialize(
        ...
        if (isCodeInRange("429", response.httpStatusCode)) {
            const body: InlineObject4 = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "InlineObject4", ""
            ) as InlineObject4;
            throw new ApiException<InlineObject4>(response.httpStatusCode, "Too many requests (rate limit exceeded)", body, response.headers);
        }
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to listRecords
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async listRecordsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<RecordList >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: RecordList = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "RecordList", ""
            ) as RecordList;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
