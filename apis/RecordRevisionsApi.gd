...
# API RecordRevisionsApi
# Instantiate this object and use it to make requests to the API.

func _bzz_get_api_name() -> String:
	return "RecordRevisionsApi"


# Operation listRecordRevisions → GET /studies/{studyKey}/recordRevisions
# List record revisions (audit trail entries) in a study
func list_record_revisions(
	...
	if page < 0:
		...
		error.identifier = "list_record_revisions.param.validation.minimum"
	...
	if size > 500:
		var error := ApiError.new()
		error.identifier = "list_record_revisions.param.validation.maximum"
	...
	var bzz_path := "/api/v1/edc/studies/{studyKey}/recordRevisions".replace("{" + "studyKey" + "}", _bzz_urlize_path_param(studyKey))
	...
	self._bzz_request(
		...
		func(bzz_response):
			bzz_response.data = RecordRevisionList.bzz_denormalize_single(bzz_response.data)
...
func list_record_revisions_threaded(
	# studyKey: String   Eg: studyKey_example
	# Study key identifying the study context for the request
	studyKey: String,
	# page: int = 0   Eg: 0
	...
	var bzz_callable := Callable(self, "list_record_revisions")
	bzz_callable.bind(
		studyKey,
		page,
		size,
