...
@interface OAIIntervalsApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation OAIIntervalsApi

NSString* kOAIIntervalsApiErrorDomain = @"OAIIntervalsApiErrorDomain";
NSInteger kOAIIntervalsApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) listIntervalsWithStudyKey: (NSString*) studyKey
    ...
    if (studyKey == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAIIntervalsApiErrorDomain code:kOAIIntervalsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/studies/{studyKey}/intervals"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAIIntervalList*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAIIntervalList*)data, error);
                                }
                            }];
}
