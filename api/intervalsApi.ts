...
export enum IntervalsApiApiKeys {
    apiKeyAuth,
    securityKeyAuth,
}
...
export class IntervalsApi {
    ...
    public setApiKey(key: IntervalsApiApiKeys, value: string) {
        (this.authentications as any)[IntervalsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async listIntervals (studyKey: string, page?: number, size?: number, sort?: string, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: IntervalList;  }> {
        const localVarPath = this.basePath + '/studies/{studyKey}/intervals'
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: IntervalList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "IntervalList");
