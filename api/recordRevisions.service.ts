...
@injectable()
export class RecordRevisionsService {
     ...
     */
    public listRecordRevisions(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'body', headers?: Headers): Observable<RecordRevisionList>;
    public listRecordRevisions(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'response', headers?: Headers): Observable<HttpResponse<RecordRevisionList>>;
    public listRecordRevisions(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe: any = 'body', headers: Headers = {}): Observable<any> {
        ...
        const response: Observable<HttpResponse<RecordRevisionList>> = this.httpClient.get(`${this.basePath}/studies/${encodeURIComponent(String(studyKey))}/recordRevisions?${queryParameters.join('&')}`, headers);
        if (observe === 'body') {
               return response.pipe(
                   map((httpResponse: HttpResponse) => <RecordRevisionList>(httpResponse.response))
               );
        }
        return response;
    }
