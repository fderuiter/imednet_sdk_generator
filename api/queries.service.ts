...
export class QueriesService {
     ...
     */
    public listQueries(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'body', headers?: Headers): Observable<QueryList>;
    public listQueries(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'response', headers?: Headers): Observable<HttpResponse<QueryList>>;
    public listQueries(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe: any = 'body', headers: Headers = {}): Observable<any> {
        ...
        const response: Observable<HttpResponse<QueryList>> = this.httpClient.get(`${this.basePath}/studies/${encodeURIComponent(String(studyKey))}/queries?${queryParameters.join('&')}`, headers);
        if (observe === 'body') {
               return response.pipe(
                   map((httpResponse: HttpResponse) => <QueryList>(httpResponse.response))
               );
        }
        return response;
    }
