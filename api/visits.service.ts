...
export class VisitsService {
     ...
     */
    public listVisits(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'body', headers?: Headers): Observable<VisitList>;
    public listVisits(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'response', headers?: Headers): Observable<HttpResponse<VisitList>>;
    public listVisits(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe: any = 'body', headers: Headers = {}): Observable<any> {
        ...
        const response: Observable<HttpResponse<VisitList>> = this.httpClient.get(`${this.basePath}/studies/${encodeURIComponent(String(studyKey))}/visits?${queryParameters.join('&')}`, headers);
        if (observe === 'body') {
               return response.pipe(
                   map((httpResponse: HttpResponse) => <VisitList>(httpResponse.response))
               );
        }
        return response;
    }
