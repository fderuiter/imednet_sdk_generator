...
export class SitesService {
     ...
     */
    public listSites(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'body', headers?: Headers): Observable<SiteList>;
    public listSites(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'response', headers?: Headers): Observable<HttpResponse<SiteList>>;
    public listSites(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe: any = 'body', headers: Headers = {}): Observable<any> {
        ...
        const response: Observable<HttpResponse<SiteList>> = this.httpClient.get(`${this.basePath}/studies/${encodeURIComponent(String(studyKey))}/sites?${queryParameters.join('&')}`, headers);
        if (observe === 'body') {
               return response.pipe(
                   map((httpResponse: HttpResponse) => <SiteList>(httpResponse.response))
               );
        }
        return response;
    }
