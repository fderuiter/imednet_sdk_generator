...
export enum RecordRevisionsApiApiKeys {
    apiKeyAuth,
    securityKeyAuth,
}

export class RecordRevisionsApi {
    ...
    public setApiKey(key: RecordRevisionsApiApiKeys, value: string) {
        (this.authentications as any)[RecordRevisionsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async listRecordRevisions (studyKey: string, page?: number, size?: number, sort?: string, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: RecordRevisionList;  }> {
        const localVarPath = this.basePath + '/studies/{studyKey}/recordRevisions'
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: RecordRevisionList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "RecordRevisionList");
