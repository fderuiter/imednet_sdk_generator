...
export class RecordsApi {
     ...
     */
    public async createRecords (studyKey: string, componentsSchemasRecordCreateRequestItem: Array<ComponentsSchemasRecordCreateRequestItem>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: RecordJobStatus;  }> {
        const localVarPath = this.basePath + '/studies/{studyKey}/records'
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(componentsSchemasRecordCreateRequestItem, "Array<ComponentsSchemasRecordCreateRequestItem>")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: RecordJobStatus;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "RecordJobStatus");
     ...
     */
    public async listRecords (studyKey: string, page?: number, size?: number, sort?: string, filter?: string, recordDataFilter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: RecordList;  }> {
        const localVarPath = this.basePath + '/studies/{studyKey}/records'
            .replace('{' + 'studyKey' + '}', encodeURIComponent(String(studyKey)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        if (recordDataFilter !== undefined) {
            localVarQueryParameters['recordDataFilter'] = ObjectSerializer.serialize(recordDataFilter, "string");
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: RecordList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "RecordList");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
