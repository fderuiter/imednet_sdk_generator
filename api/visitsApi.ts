...
export class VisitsApi {
    ...
    public setApiKey(key: VisitsApiApiKeys, value: string) {
        (this.authentications as any)[VisitsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async listVisits (studyKey: string, page?: number, size?: number, sort?: string, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: VisitList;  }> {
        const localVarPath = this.basePath + '/studies/{studyKey}/visits'
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: VisitList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "VisitList");
