...
export class CodingsService {
     ...
     */
    public listCodings(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'body', headers?: Headers): Observable<CodingList>;
    public listCodings(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe?: 'response', headers?: Headers): Observable<HttpResponse<CodingList>>;
    public listCodings(studyKey: string, page?: number, size?: number, sort?: string, filter?: string, observe: any = 'body', headers: Headers = {}): Observable<any> {
        ...
        const response: Observable<HttpResponse<CodingList>> = this.httpClient.get(`${this.basePath}/studies/${encodeURIComponent(String(studyKey))}/codings?${queryParameters.join('&')}`, headers);
        if (observe === 'body') {
               return response.pipe(
                   map((httpResponse: HttpResponse) => <CodingList>(httpResponse.response))
               );
        }
        return response;
    }
