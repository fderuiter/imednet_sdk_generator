...
final api = Openapi().getRecordsApi();
final String studyKey = studyKey_example; // String | Study key identifying the study context for the request
final BuiltList<ComponentsSchemasRecordCreateRequestItem> componentsSchemasRecordCreateRequestItem = ; // BuiltList<ComponentsSchemasRecordCreateRequestItem> | An array of record objects defining the operation to perform (subject registration, record creation, or scheduled record update).

try {
    final response = api.createRecords(studyKey, componentsSchemasRecordCreateRequestItem);
    print(response);
 ...
 **componentsSchemasRecordCreateRequestItem** | [**BuiltList&lt;ComponentsSchemasRecordCreateRequestItem&gt;**](ComponentsSchemasRecordCreateRequestItem.md)| An array of record objects defining the operation to perform (subject registration, record creation, or scheduled record update). | 
...
final String recordDataFilter = recordDataFilter_example; // String | Optional filter criteria to apply on recordData fields (question responses). Supports ==, !=, <, <=, >, >=, =~ (contains) and can combine multiple criteria with `;` (AND) or `,` (OR).

try {
    final response = api.listRecords(studyKey, page, size, sort, filter, recordDataFilter);
    print(response);
} catch on DioException (e) {
