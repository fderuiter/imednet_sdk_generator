...
# **listForms**
```swift
    open class func listForms(studyKey: String, page: Int? = nil, size: Int? = nil, sort: String? = nil, filter: String? = nil, completion: @escaping (_ data: FormList?, _ error: Error?) -> Void)
```
...
// List forms in a study
FormsAPI.listForms(studyKey: studyKey, page: page, size: size, sort: sort, filter: filter) { (response, error) in
