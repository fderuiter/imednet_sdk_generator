...
# **listStudies**
```swift
    open class func listStudies(page: Int? = nil, size: Int? = nil, sort: String? = nil, filter: String? = nil, completion: @escaping (_ data: StudyList?, _ error: Error?) -> Void)
```
...
// List studies accessible by API key
StudiesAPI.listStudies(page: page, size: size, sort: sort, filter: filter) { (response, error) in
