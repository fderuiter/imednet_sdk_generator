...
```php
listSubjects($study_key, $page, $size, $sort, $filter): \OpenAPI\Client\Model\SubjectList
```
...
$apiInstance = new OpenAPI\Client\Api\SubjectsApi(
...
try {
    $result = $apiInstance->listSubjects($study_key, $page, $size, $sort, $filter);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling SubjectsApi->listSubjects: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\SubjectList**](../Model/SubjectList.md)
