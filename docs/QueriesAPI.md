...
# **listQueries**
```swift
    open class func listQueries(studyKey: String, page: Int? = nil, size: Int? = nil, sort: String? = nil, filter: String? = nil, completion: @escaping (_ data: QueryList?, _ error: Error?) -> Void)
```
...
// List data queries in a study
QueriesAPI.listQueries(studyKey: studyKey, page: page, size: size, sort: sort, filter: filter) { (response, error) in
