...
# **list_variables**
> VariableList list_variables(study_key, page=page, size=size, sort=sort, filter=filter)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # List variables (fields) in a study
        api_response = api_instance.list_variables(study_key, page=page, size=size, sort=sort, filter=filter)
