...
# **list_variables**
> VariableList list_variables(study_key => $study_key, page => $page, size => $size, sort => $sort, filter => $filter)
...
eval {
    my $result = $api_instance->list_variables(study_key => $study_key, page => $page, size => $size, sort => $sort, filter => $filter);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling VariablesApi->list_variables: $@\n";
}
```
