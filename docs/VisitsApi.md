...
# **list_visits**
> VisitList list_visits(study_key, page=page, size=size, sort=sort, filter=filter)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    api_instance = openapi_client.VisitsApi(api_client)
    ...
    try:
        # List visits (subject visit instances) in a study
        api_response = api_instance.list_visits(study_key, page=page, size=size, sort=sort, filter=filter)
        print("The response of VisitsApi->list_visits:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling VisitsApi->list_visits: %s\n" % e)
```
