# WWW::OpenAPIClient::VisitsApi

## Load the API package
```perl
use WWW::OpenAPIClient::Object::VisitsApi;
```
...
# **list_visits**
> VisitList list_visits(study_key => $study_key, page => $page, size => $size, sort => $sort, filter => $filter)
...
my $api_instance = WWW::OpenAPIClient::VisitsApi->new(
...
eval {
    my $result = $api_instance->list_visits(study_key => $study_key, page => $page, size => $size, sort => $sort, filter => $filter);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling VisitsApi->list_visits: $@\n";
}
```
