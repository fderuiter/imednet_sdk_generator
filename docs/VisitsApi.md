...
try {
    val result : VisitList = apiInstance.listVisits(studyKey, page, size, sort, filter)
    println(result)
} catch (e: ClientException) {
    println("4xx response calling VisitsApi#listVisits")
    e.printStackTrace()
} catch (e: ServerException) {
    println("5xx response calling VisitsApi#listVisits")
    e.printStackTrace()
}
```
