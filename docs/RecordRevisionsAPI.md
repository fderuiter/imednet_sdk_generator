...
# **listRecordRevisions**
```swift
    open class func listRecordRevisions(studyKey: String, page: Int? = nil, size: Int? = nil, sort: String? = nil, filter: String? = nil, completion: @escaping (_ data: RecordRevisionList?, _ error: Error?) -> Void)
```
...
// List record revisions (audit trail entries) in a study
RecordRevisionsAPI.listRecordRevisions(studyKey: studyKey, page: page, size: size, sort: sort, filter: filter) { (response, error) in
