...
|------------- | ------------- | -------------|
|[**listRecordRevisions**](#listrecordrevisions) | **GET** /studies/{studyKey}/recordRevisions | List record revisions (audit trail entries) in a study|
...
const { status, data } = await apiInstance.listRecordRevisions(
    studyKey,
    page,
    size,
    sort,
