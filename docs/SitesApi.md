...
# **list_sites**
> SiteList list_sites(study_key, page=page, size=size, sort=sort, filter=filter)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # List sites for a study
        api_response = api_instance.list_sites(study_key, page=page, size=size, sort=sort, filter=filter)
