# WWW::OpenAPIClient::QueriesApi

## Load the API package
```perl
use WWW::OpenAPIClient::Object::QueriesApi;
```
...
# **list_queries**
> QueryList list_queries(study_key => $study_key, page => $page, size => $size, sort => $sort, filter => $filter)
...
eval {
    my $result = $api_instance->list_queries(study_key => $study_key, page => $page, size => $size, sort => $sort, filter => $filter);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling QueriesApi->list_queries: $@\n";
}
```
