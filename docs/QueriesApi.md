...
# **list_queries**
> QueryList list_queries(study_key, page=page, size=size, sort=sort, filter=filter)
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # List data queries in a study
        api_response = api_instance.list_queries(study_key, page=page, size=size, sort=sort, filter=filter)
