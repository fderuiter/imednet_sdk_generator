...
# **listVisits**
```swift
    open class func listVisits(studyKey: String, page: Int? = nil, size: Int? = nil, sort: String? = nil, filter: String? = nil, completion: @escaping (_ data: VisitList?, _ error: Error?) -> Void)
```
...
// List visits (subject visit instances) in a study
VisitsAPI.listVisits(studyKey: studyKey, page: page, size: size, sort: sort, filter: filter) { (response, error) in
