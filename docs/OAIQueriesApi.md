...
------------- | ------------- | -------------
[**listQueries**](OAIQueriesApi.md#listqueries) | **GET** /studies/{studyKey}/queries | List data queries in a study
...
OAIQueriesApi*apiInstance = [[OAIQueriesApi alloc] init];

// List data queries in a study
[apiInstance listQueriesWithStudyKey:studyKey
              page:page
              size:size
              sort:sort
              filter:filter
          completionHandler: ^(OAIQueryList* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling OAIQueriesApi->listQueries: %@", error);
                        }
                    }];
```
